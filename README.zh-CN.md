...
轮流刷新。平衡档每秒产生 60 个 symbol、每码 1000 字节；扣除 20 字节帧头后，每码携带
980 字节编码块。QR 纠错级别为 L。页面只提供一个 shadcn/ui 速度滑块，将二维码密度和更新率
...
| 档位 | 本机推荐边界 | 原始吞吐模型 |
|---|---|---:|
| 稳定 | CPU、刷新率或物理像素不足 | 约 20 KiB/s |
| 平衡 | 4+ 逻辑线程、约 45+ Hz、物理短边 1024px+ | 约 57 KiB/s |
| 高速 | 8+ 逻辑线程、约 55+ Hz、物理短边 1800px+ | 约 135 KiB/s |
...
- Worker 池：普通帧按帧号轮询 4 个带重叠的网格区域，每次只搜索 1 个 symbol；连续失败时
  才对整张捕获画面做最多 4 码的 Robust 回退。
...
平衡档参数为 `1 × 60 = 60` symbols/s，`blockLength = 1000 - 20 = 980` 字节，对应
`57.421875 KiB/s` 的原始载荷理论上限。这是模型而非实测结果。例如：
